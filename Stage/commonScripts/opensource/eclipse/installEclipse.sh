...
ECLIPSE_HOME=$OS_BASE/$ECLIPSE_INSTALL_NAME

#
echo "Checking Eclipse Home: "$ECLIPSE_HOME
if [ ! -f "$ECLIPSE_HOME/bin/soapui.sh" ]; then
 #
  # Install Eclipse
  ...
  echo Move $ECLIPSE_INSTALL_TMP/* to $ECLIPSE_HOME
  mv  $ECLIPSE_INSTALL_TMP/* $ECLIPSE_HOME
