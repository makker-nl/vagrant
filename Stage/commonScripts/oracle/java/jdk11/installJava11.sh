#!/bin/bash
#
# Download the Linux X64 Compressed Archive from https://www.oracle.com/java/technologies/downloads/#java11
#
...
if [ ! -f "$JAVA_HOME/bin/java" ]; then
  if [[ -z "$JAVA_INSTALL_TAR" ]]; then
    ...
    echo Try to install Jdk11 from $JAVA_INSTALL_HOME/$JAVA_INSTALL_TAR
	  echo create folder $JAVA_INSTALL_TMP
	  mkdir -p $JAVA_INSTALL_TMP
	  echo create JAVA_HOME $JAVA_HOME
	  mkdir -p $JAVA_HOME
	  echo Untar $JAVA_INSTALL_HOME/$JAVA_INSTALL_TAR to $JAVA_INSTALL_TMP
	  tar -xf $JAVA_INSTALL_HOME/$JAVA_INSTALL_TAR -C $JAVA_INSTALL_TMP
    JAVA_INSTALL_NAME=$(find $JAVA_INSTALL_TMP -type d -name $JDK_BASE_NAME | awk -F"/" '{print $NF}' | sort -r | head -1)
