#!/bin/bash
#
# Download the Linux X64 Compressed Archive from https://www.oracle.com/java/technologies/downloads/#java17
#
SCRIPTPATH=$(dirname $0)
#
. $SCRIPTPATH/../../../install_env.sh
#
JAVA_HOME=$ORACLE_BASE/product/jdk17
JAVA_INSTALL_HOME=$INSTALL_HOME/Oracle/Java
JAVA_INSTALL_TMP=$INSTALL_TMP_DIR/jdk
JDK_BASE_NAME=jdk-17*
...
function install_java(){
  ...
  if [ ! -f "$JAVA_HOME/bin/java" ]; then
    if [[ -z "$JAVA_INSTALL_TAR" ]]; then
      ...
      echo Try to install jdk17 from $JAVA_INSTALL_HOME/$JAVA_INSTALL_TAR
